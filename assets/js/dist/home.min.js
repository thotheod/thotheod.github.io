/*!
 * jekyll-theme-chirpy v7.2.4 | © 2019 Cotes Chung | MIT Licensed | https://github.com/cotes2020/jekyll-theme-chirpy/ this is it
 */
!(function () {
  'use strict';
  const t = new Map();
  ...
  class Qe extends N {
    ...
    dispose() {
      clearTimeout(this._timeout),
        S.off(this._element.closest(ze), qe, this._hideModalHandler),
    ...
    _setListeners() {
      ...
      for (const e of t)
        if ('click' === e)
          ...
          S.on(this._element, t, this._config.selector, (t) => {
            ...
            (e._activeTrigger['focusin' === t.type ? Ve : We] = !0), e._enter();
          }),
            S.on(this._element, n, this._config.selector, (t) => {
              const e = this._initializeOnDelegatedTarget(t);
              (e._activeTrigger['focusout' === t.type ? Ve : We] =
      ...
      (this._hideModalHandler = () => {
        ...
        S.on(this._element.closest(ze), qe, this._hideModalHandler);
    }
    _fixTitle() {
  ...
  function tn() {
    this.complete &&
      (this.hasAttribute(Ge)
        ? Ze.call(this, Je.BLUR)
        : Ze.call(this, Je.SHIMMER));
  }
  function en() {
    const t = this.getAttribute(Xe);
    this.setAttribute('src', encodeURI(t)), this.removeAttribute(Xe);
  }
  class nn {
  ...
  var sn;
  const an = document.getElementById('toc-bar'),
    ln = document.getElementById('toc-solo-trigger'),
    cn = document.getElementsByClassName('toc-trigger'),
    un = document.getElementById('toc-popup'),
    fn = document.getElementById('toc-popup-close'),
    dn = 'overflow-hidden',
    pn = 'closing';
  class hn {
    static initBar() {
      new IntersectionObserver(
        (t) => {
          t.forEach((t) => {
            an.classList.toggle('invisible', t.isIntersecting);
          });
        },
        { rootMargin: `-${on(hn, this, gn)._}px 0px 0px 0px` }
      ).observe(ln),
        (mn._ = on(hn, this, !1));
    }
    ...
    static refresh() {
      on(hn, this, mn)._ && this.initComponents(),
    ...
    static showPopup() {
      this.lockScroll(!0), un.showModal();
      un.querySelector('li.is-active-li').scrollIntoView({ block: 'center' });
    }
    static hidePopup() {
      un.toggleAttribute(pn),
        un.addEventListener(
          'animationend',
          () => {
            un.toggleAttribute(pn), un.close();
          },
          { once: !0 }
        ),
        this.lockScroll(!1);
    ...
    static lockScroll(t) {
      document.documentElement.classList.toggle(dn, t),
        document.body.classList.toggle(dn, t);
    }
    static clickBackdrop(t) {
      if (un.hasAttribute(pn)) return;
    ...
    static initComponents() {
      this.initBar(),
        ...
        (un.onclick = (t) => this.clickBackdrop(t)),
        (fn.onclick = () => this.hidePopup()),
        (un.oncancel = (t) => {
  ...
  rn(hn, 'options', {
    ...
    scrollSmooth: !1,
    collapseDepth: 4,
    headingsOffset: on((sn = hn), sn, gn)._
  });
  rn(
    class {
      static refresh() {
  ...
  const vn = document.getElementById('mode-toggle');
  const bn = document.getElementById('sidebar'),
    yn = document.getElementById('sidebar-trigger'),
    _n = document.getElementById('mask');
  class wn {
    static toggle() {
      (En._ = on(wn, this, !on(wn, this, En)._)),
        document.body.toggleAttribute('sidebar-display', on(wn, this, En)._),
        bn.classList.toggle('z-2', on(wn, this, En)._),
        _n.classList.toggle('d-none', !on(wn, this, En)._);
    }
  }
  var En = { _: !1 };
  const On = document.getElementById('sidebar-trigger'),
    xn = document.getElementById('search-trigger'),
    An = document.getElementById('search-cancel'),
    Cn = document.querySelectorAll('#main-wrapper>.container>.row'),
    Tn = document.getElementById('topbar-title'),
    Ln = document.getElementById('search'),
    Sn = document.getElementById('search-result-wrapper'),
    jn = document.getElementById('search-results'),
    kn = document.getElementById('search-input'),
    Dn = document.getElementById('search-hints'),
    Pn = 'd-block',
    Mn = 'd-none',
    Nn = 'input-focus',
    Bn = 'd-flex';
  class In {
    ...
    static off() {
      An.classList.remove(Pn),
        Ln.classList.remove(Bn),
        On.classList.remove(Mn),
        Tn.classList.remove(Mn),
        xn.classList.remove(Mn);
    }
  }
  class Fn {
    static on() {
      this.resultVisible ||
        (Sn.classList.remove(Mn),
        Cn.forEach((t) => {
          t.classList.add(Mn);
        }),
    ...
    static off() {
      this.resultVisible &&
        ((jn.innerHTML = ''),
        Dn.classList.contains(Mn) && Dn.classList.remove(Mn),
        Sn.classList.add(Mn),
        Cn.forEach((t) => {
  ...
  function Hn() {
    return An.classList.contains(Pn);
  }
  rn(Fn, 'resultVisible', !1),
    (function () {
      const t = document.querySelectorAll('article img');
      if (0 === t.length) return;
      t.forEach((t) => {
        t.addEventListener('load', tn);
      }),
        document
          .querySelectorAll('article img[loading="lazy"]')
          .forEach((t) => {
            t.complete && Ze.call(t, Je.SHIMMER);
          });
      const e = document.querySelectorAll(`article img[${Ge}="true"]`);
      e.length &&
        e.forEach((t) => {
          en.call(t);
        });
    })(),
    dayjs.locale(nn.locale),
    dayjs.extend(window.dayjs_plugin_localizedFormat),
    document.querySelectorAll(`[${nn.attrTimestamp}]`).forEach((t) => {
      const e = dayjs.unix(nn.getTimestamp(t)),
        n = e.format(nn.getDateFormat(t));
      if (
        ((t.textContent = n),
        t.removeAttribute(nn.attrTimestamp),
        t.removeAttribute(nn.attrDateFormat),
    ...
    (yn.onclick = _n.onclick = () => wn.toggle()),
    xn.addEventListener('click', () => {
      In.on(), Fn.on(), kn.focus();
    }),
    An.addEventListener('click', () => {
      In.off(), Fn.off();
    }),
    kn.addEventListener('focus', () => {
      Ln.classList.add(Nn);
    }),
    kn.addEventListener('focusout', () => {
      Ln.classList.remove(Nn);
    }),
    kn.addEventListener('input', () => {
      '' === kn.value
        ? Hn()
          ? Dn.classList.remove(Mn)
          : Fn.off()
        : (Fn.on(), Hn() && Dn.classList.add(Mn));
    }),
    vn &&
